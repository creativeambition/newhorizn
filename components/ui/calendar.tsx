...
function Calendar({
  ...
  const DayContent = (dayProps: DayContentProps) => {
    ...
    const isBuffer = bufferDates.some(d => d.toISOString().split("T")[0] === dateStr);
    ...
    return (
      <div className="relative w-full h-full flex items-center justify-center">
        ...
        {isBuffer && !isBooked && (
          <div className="absolute top-0.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-yellow-500" />
        )}
      </div>
    );
  };
